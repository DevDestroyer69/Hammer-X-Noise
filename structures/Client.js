...
module.exports = class TysonClient extends Client {
  ...
  async connectMongo() {
    const connection = await connect(this.config.mongoURI, { useNewUrlParser: true, useUnifiedTopology: true });
  ...
  async send(channelID, options) {
    ...
    return await channel.send(options).catch(e => this.logger.log(e));
    //return await channel.send({ content: `${content}`, embeds: [embed] }).catch(e => this.logger.log(e));
  };
